...
<div class="flex items-center gap-4 border-b-2">
    <h1 class="text-5xl  pb-6 jus">Category</h1>
    <p class="text-base text-dark-variant relative bottom-2">Settings | Category</p>
</div>
<!-- form message cell  -->
<?= view_cell('UpdateSuccessCell', 'type=Category') ?>
<?= view_cell('CreateSuccessCell', 'type=Category') ?>
<?= view_cell('FormErrorCell') ?>
<!-- delete message cell  -->
<?= view_cell('DeleteSuccessCell', 'type=Category') ?>
<div class="mt-24">
    ...
    <div class="absolute p-4 w-full max-w-md max-h-full left-1/2 -translate-x-1/2">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-4 md:p-5">
                <form id="create-category" method="post" class="mt-4">
                    <div class="grid gap-y-2">
                        <label for="category_code">Category Code</label>
                        <input class="px-4 py-2 bg-dark-variant-2 rounded-xl outline-accent" type="text" name="category_code">
                    </div>
                    <div class="grid mt-2 gap-y-2">
                        <label for="category_name">Category Name</label>
                        <input class="px-4 py-2 bg-dark-variant-2 rounded-xl outline-accent" type="text" name="category_name">
                    </div>
...
<div id="update" tabindex="-1" aria-hidden="true" class="overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem) before:fixed before:inset-0 before:w-full before:h-full before:bg-[rgba(0,0,0,0.5)] max-h-full update-modal hidden">
    <div class="absolute p-4 w-full max-w-md max-h-full left-1/2 -translate-x-1/2">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <!-- Modal body -->
            <div class="p-4 md:p-5">
                <form id="update-category" method="post" class="mt-4">
                    <input type="hidden" id="id" name="id">
                    <div class="grid gap-y-2">
                        <label for="category_code">Category Code</label>
                        <input class="px-4 py-2 bg-dark-variant-2 rounded-xl outline-accent" id="update-category-code" type="text" name="category_code">
                    </div>
                    <div class="grid mt-2 gap-y-2">
                        <label for="category_name">Category Name</label>
                        <input class="px-4 py-2 bg-dark-variant-2 rounded-xl outline-accent" id="update-category-name" type="text" name="category_name">
                    </div>
                    <button type="submit" class="btn mt-4">Submit</button>
                </form>
            </div>
...
<!-- delete modal  -->
<?= view_cell('ModalDeleteCell', 'type=category') ?>

<script src="<?= base_url() ?>js/settings/category.js"></script>
